...
class PersonTwoSocketRecorderController extends GetxController {
  ...
  void recordVoice() {
    Permission.microphone.request().then((permission) {
      if (permission.isGranted) {
        ...
        asrToSend['config']['language']['sourceLanguage'] = Get.find<PersonTwoUIController>().currentSelectedLanguageCode;
        translationToSend['config']['language']['sourceLanguage'] = Get.find<PersonTwoUIController>().currentSelectedLanguageCode;
        translationToSend['config']['language']['targetLanguage'] = Get.find<PersonOneUIController>().currentSelectedLanguageCode;
        ttsToSend['config']['language']['sourceLanguage'] = Get.find<PersonOneUIController>().currentSelectedLanguageCode;
        ttsToSend['config']['gender'] = Get.find<PersonTwoUIController>().isFemaleBtnSelected ? 'female' : 'male';
        ...
        _socketConnect.recordVoice(socketTaskSeqToSend: socketTaskSeqToSend, isReqForPerOneAtBottom: false);
      } else {}
    });
  }
